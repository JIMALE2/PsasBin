...
forceinline word_t ROT(word_t x, int n) { return x >> n | x << (64 - n); }
...
forceinline int NOTZERO(word_t a, word_t b) {
  ...
  result |= result >> 16;
  result |= result >> 8;
  //printf ("Not zero here");
  return ((((int)(result & 0xff) - 1) >> 8) & 1) - 1;
}

forceinline word_t LPAD(int i) { return 0x80ull << (56 - 8 * i); }
...
/*forceinline  static void printline (uint64_t xx){
...
forceinline word_t RET(const uint8_t* bytes, int n) {
...
forceinline word_t RET2(const uint64_t* bytes, int n) {
  //bytes=malloc(sizeof(uint8_t*));
  uint64_t x = *(uint64_t*)bytes & MASK(n);
  return U64BIG(x);
  xx=U64BIG(x);
...
forceinline void STORE(uint8_t* bytes, word_t w, int n) {
  ...
  *(uint64_t*)bytes |= U64BIG(w);
  //for (int i;i<20;i++){
  //printf("bytes : %016I64X\n", bytes[i]);}

}

forceinline word_t RETBYTES(const uint8_t* bytes, int n) {
  uint64_t x = 0;
